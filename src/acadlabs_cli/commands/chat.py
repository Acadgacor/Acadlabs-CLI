...
console = Console()
app = typer.Typer(name="chat", help="Chat commands")
...
def _show_token_status(
    ...
    if history_tokens >= token_manager.danger_threshold:
        console.print("\n[red bold]DANGER: Context hampir penuh! Ketik 'clear' untuk clear context.[/red bold]")
    elif history_tokens >= token_manager.critical_threshold:
        console.print("\n[orange3 bold]CRITICAL: Context sudah tinggi. Pertimbangkan clear context.[/orange3 bold]")
    elif history_tokens >= token_manager.warning_threshold:
        console.print("\n[yellow bold]WARNING: Context mulai panjang. Biaya akan meningkat.[/yellow bold]")
    else:
